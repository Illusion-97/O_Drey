...
public class Odrey extends ListenerAdapter {
    ...
    private Role banned;
    public Odrey(SimpMessagingTemplate messagingTemplate, @Value("${discord.token}") String key, ApplicationEventPublisher publisher, ThreadPoolTaskScheduler scheduler) {
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        if(message.getContentRaw().contains("Miaouh") && target != null) {
    ...
    public void onLoginEvent(LoginEvent event) {
        findUser(event.getUsername())
                .ifPresent(user -> user.openPrivateChannel().queue(privateChannel ->
    ...
    public Optional<User> findUser(String name) {
        return jda.getGuilds().stream()
                .map(guild -> guild.getMembersByName(name, true))
                .filter(list -> !list.isEmpty())
                .findFirst()
                .map(list -> list.get(0).getUser());
    }

    @Override
    public void onButtonInteraction(ButtonInteractionEvent event) {
        User user = event.getUser();
        if (event.getButton().getId().equals("OK")) {
            String destination = "/login/" + user.getName();
            event.reply("Décision prise en compte !").setEphemeral(true).queue(r ->
                    {
